...
class PySAR():
    ...
    def encode_aai(self, indices=None, show_plot=True, print_results=True, output_folder=""):
        """
    ...
    def get_descriptor_encoding(self, descriptors=None):
        ...
        # 重置DataFrame的索引
        for i in range(len(encoded_desc_vals)):
            encoded_desc_vals[i] = encoded_desc_vals[i].reset_index(drop=True)
        #concatenate dataframes of descriptors
        encoded_desc = pd.concat(encoded_desc_vals, axis=1)

        return encoded_desc

    def encode_descriptor(self, descriptor=None, show_plot=True, print_results=True, output_folder=""):
        """
    ...
    def encode_aai_descriptor(self, indices=None, descriptors=None, show_plot=True, print_results=True,
                              output_folder="",idx_flag=False,idx=[1]):
        """
