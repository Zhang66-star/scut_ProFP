...
class Model():
    ...
    def __init__(self, X, Y, algorithm, parameters={}, test_split=0.2):
        ...
        self.valid_models = ['plsregression', 'randomforestregressor', 'adaboostregressor',\
                            ...
                            'stochasticgradientdescent', 'kneighborsregressor', 'knearestneighbors', 'knn','xgb','lgbm']
    ...
    def get_model(self):
        ...
        if (self.algorithm.lower().strip() == 'plsregression'):
           ...
           self.algorithm.lower().strip() == 'knearestneighbors'):
        ...
        elif (self.algorithm.lower().strip() == 'xgb'):

            for k,v in XGBRegressor().__dict__.items(): 
                if (k in list(self.parameters.keys())): parameters[k] = self.parameters[k]
            ...
            if (parameters != {} or parameters != []):
                ...
                model = XGBRegressor()
        elif (self.algorithm.lower().strip() == 'lgbm'):

            for k,v in LGBMRegressor().__dict__.items(): 
                if (k in list(self.parameters.keys())): parameters[k] = self.parameters[k]

            if (parameters != {} or parameters != []):
                model = LGBMRegressor(**self.parameters)
            else:
                model = LGBMRegressor()
    ...
    def predict_new(self,inpu):
        res=self.model_fit.predict(inpu)
        return res
