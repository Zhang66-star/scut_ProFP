...
class Descriptors():
    ...
    """
    def __init__(self, config_file="", protein_seqs=None,imp=True):
        ...
        if ((os.path.isfile(self.desc_parameters.descriptors_csv) or 
            os.path.isfile((os.path.join(DATA_DIR, self.desc_parameters.descriptors_csv))) )and
            imp==True):
            # print('import')
    ...
    def get_amino_acid_composition(self,iftest=False):
        """
    ...
    def get_dipeptide_composition(self,iftest=False):
        """
    ...
    def get_tripeptide_composition(self,iftest=False):
        """ 
    ...
    def get_moreaubroto_autocorrelation(self,iftest=False):
        """
    ...
    def get_moran_autocorrelation(self,iftest=False):
        """
    ...
    def get_geary_autocorrelation(self,iftest=False):
        """
    ...
    def get_ctd_composition(self,iftest=False):
        """ 
    ...
    def get_conjoint_triad(self,iftest=False):
        """
    ...
    def get_sequence_order_coupling_number(self,iftest=False):
        """
    ...
    def get_descriptor_encoding_pred(self, descriptor,iftest=False):
        ...
        if (desc == 'amino_acid_composition'):
            if (getattr(self, desc).empty):
                self.get_amino_acid_composition(iftest)
            desc_encoding = self.amino_acid_composition

        elif (desc == 'dipeptide_composition'):
            if (getattr(self, desc).empty):
                self.get_dipeptide_composition(iftest)
            desc_encoding = self.dipeptide_composition

        elif (desc == 'tripeptide_composition'):
            if (getattr(self, desc).empty):
                self.get_tripeptide_composition(iftest)
            desc_encoding = self.tripeptide_composition

        elif (desc == 'moreaubroto_autocorrelation'):
            if (getattr(self, desc).empty):
              self.get_moreaubroto_autocorrelation(iftest)
            desc_encoding = self.moreaubroto_autocorrelation
            
        elif (desc == 'moran_autocorrelation'):
            if (getattr(self, desc).empty):
              self.get_moran_autocorrelation(iftest)
            desc_encoding = self.moran_autocorrelation

        elif (desc == 'geary_autocorrelation'):
            if (getattr(self, desc).empty):
              self.get_geary_autocorrelation(iftest)
            desc_encoding = self.geary_autocorrelation
            ...
            if (getattr(self, desc).empty):
              self.get_ctd_composition(iftest)
            desc_encoding = self.ctd_composition

        elif (desc == 'ctd_transition'):
            if (getattr(self, desc).empty):
              self.get_ctd_transition(iftest)
            desc_encoding = self.ctd_transition

        elif (desc == 'ctd_distribution'):
            if (getattr(self, desc).empty):
              self.get_ctd_distribution(iftest)
            desc_encoding = self.ctd_distribution

        elif (desc == 'conjoint_triad'):
            if (getattr(self, desc).empty):
              self.get_conjoint_triad(iftest)
            desc_encoding = self.conjoint_triad

        elif (desc == 'sequence_order_coupling_number'):
            if (getattr(self, desc).empty):
              self.get_sequence_order_coupling_number(iftest)
            desc_encoding = self.sequence_order_coupling_number

        elif (desc == 'quasi_sequence_order'):
            if (getattr(self, desc).empty):
              self.get_quasi_sequence_order(iftest)
            desc_encoding = self.quasi_sequence_order

        elif (desc == 'pseudo_amino_acid_composition'):
            if (getattr(self, desc).empty):
              self.get_pseudo_amino_acid_composition(iftest)
            desc_encoding = self.pseudo_amino_acid_composition

        elif (desc == 'amphiphilic_pseudo_amino_acid_composition'):
            if (getattr(self, desc).empty):
              self.get_amphiphilic_pseudo_amino_acid_composition(iftest)
            desc_encoding = self.amphiphilic_pseudo_amino_acid_composition
        else:
          desc_encoding = None           #no matching descriptor found
