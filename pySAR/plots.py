...
def plot_reg(Y_true, Y_pred, r2, output_folder="", show_plot=False):
    ...
    # ax = sns.jointplot(x=Y_true, y=Y_pred, kind='reg')
    # plt.show()
    ax = sns.regplot(x=Y_true, y=Y_pred, marker="o", truncate=False, fit_reg=True)
    r2_annotation = 'R2: {:.3f} '.format(r2)
    # ax.ax_joint.text(0.15, 0.92, r2_annotation, ha="left", va="top", fontsize=12, color="blue",
    #     fontweight="bold")
    ax.text(0.15, 0.92, r2_annotation, ha="left", va="top", fontsize=12, color="blue",
    ...
    plt.title('Observed vs Predicted Ftitness',fontdict=dict(weight='bold'), fontsize=15)
